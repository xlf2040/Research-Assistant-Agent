const GPTResearcher = (() => {
  ...
  const updateWebSocketStatus = () => {
    ...
    if (researchStatusEl) {
      researchStatusEl.textContent = isResearchActive ? '进行中' : '未启动';
    }
  ...
  const clearConversationHistory = () => {
    if (confirm('确定要清除所有研究历史吗？此操作不可撤销。')) {
      conversationHistory = [];
      saveConversationHistory();
      renderHistoryEntries();
      showToast('研究历史已清除');
    ...
    conversationHistory.forEach((entry, index) => {
      ...
      entryElement.innerHTML = `
        ...
        <div class="history-entry-actions">
          <button class="history-entry-action delete-entry" title="删除此研究记录"><i class="fas fa-trash-alt"></i></button>
        </div>
      `;
...
- Click Cancel to REPLACE all existing history with imported data`)) {
  ...
  const reconnectWebSocket = (message = null) => {
    ...
    addChatMessage(`连接断开，正在尝试重连（${reconnectAttempts}/${maxReconnectAttempts}）...`, false);
  ...
  const createMCPInfoModal = () => {
    ...
    modal.innerHTML = `
      <div class="mcp-info-content">
        ...
        <p>MCP 使本应用能够通过标准化协议连接外部工具和数据源。</p>
        ...
        <ul>
          ...
          <li><span class="highlight">保持安全：</span>通过身份认证进行可控访问</li>
        </ul>

        <h4 class="highlight">快速上手：</h4>
        <ul>
          <li>勾选上方复选框启用 MCP</li>
          <li>点击预设将预配置的服务器添加到 JSON</li>
        ...
        <ul>
          ...
          <li><span class="highlight">command：</span>运行服务器的命令（如 "npx", "python"）</li>
