      ...
      >
        ...
        <div 
          className={`h-full transition-all duration-300 text-white overflow-hidden 
            ${isOpen 
              ? 'bg-gray-900/85 shadow-2xl shadow-black/30 p-3 sm:p-4' 
              : 'bg-transparent p-0'
            }`}
        >
                ...
                <div className="overflow-y-auto h-[calc(100vh-150px)] sm:h-[calc(100vh-190px)] pr-1 custom-scrollbar">
                  {history.length === 0 ? (
                    <div className="text-center py-8 sm:py-10 px-4">
                      ...
                      <p className="text-sm text-gray-400">开始第一次研究，建立您的知识库</p>
                    </div>
                  ) : (
                    <ul className="space-y-2 sm:space-y-3">
                      {history.map((item) => (
                        <motion.li 
                          key={item.id}
                          className="relative rounded-xl transition-all duration-300 overflow-hidden group bg-gray-900/40 hover:bg-gray-800/60 border border-gray-700/30 hover:border-gray-600/50"
