    ...
    async def conduct_research(self):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "starting_research",
                f"[START] 开始执行研究任务: '{self.researcher.query}'...",
                self.researcher.websocket,
            )
            await stream_output(
                "logs",
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"研究步骤已完成.\n[COST] 研究总费用: ${self.researcher.get_costs()}",
                self.researcher.websocket,
            )
            if self.json_handler:
    ...
    async def _get_context_by_vectorstore(self, query, filter: dict | None = None):
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "subqueries",
                f"[QUERIES] 将基于以下子查询开展研究: {sub_queries}...",
                self.researcher.websocket,
                True,
                sub_queries,
            )
            ...
            if mcp_strategy == "disabled":
                ...
                if self.researcher.verbose:
                    await stream_output(
                        "logs",
                        "mcp_optimization",
                        f"[FAST] MCP 快速模式: 仅对主查询执行一次 (性能优化)",
                        self.researcher.websocket,
                    )
                ...
                if self.researcher.verbose:
                    await stream_output(
                        "logs",
                        "mcp_comprehensive",
                        f"[SEARCH] MCP 深度模式: 将对每个子查询执行 (全面模式)",
                        self.researcher.websocket,
                    )
                ...
                try:
                    ...
                    if mcp_results:
                        ...
                        if self.researcher.verbose:
                            await stream_output(
                                "logs",
                                "mcp_results_cached",
                                f"[OK] 缓存了查询 {i}/{len(queries)} 的 {len(mcp_results)} 条 MCP 结果",
                                self.researcher.websocket,
                            )
                except Exception as e:
                    ...
                    if self.researcher.verbose:
                        await stream_output(
                            "logs",
                            "mcp_cache_error",
                            f"[WARN] 查询 {i} 的 MCP 研究出错, 继续使用其他来源",
                            self.researcher.websocket,
                        )
        ...
        try:
                ...
                if mcp_strategy == "disabled":
                    ...
                    if self.researcher.verbose:
                        await stream_output(
                            "logs",
                            "mcp_cache_reuse",
                            f"[CACHE] 复用缓存的 MCP 结果 ({len(mcp_context)} 条来源) 用于: {sub_query}",
                            self.researcher.websocket,
                        )
                    ...
                    if self.researcher.verbose:
                        await stream_output(
                            ...
                            f"[SEARCH] 正在执行深度 MCP 研究: {sub_query}",
                            self.researcher.websocket,
                        )
                    ...
                    if self.researcher.verbose:
                        await stream_output(
                            "logs",
                            "mcp_fallback",
                            f"[MCP] MCP 缓存不可用, 正在执行 MCP 研究: {sub_query}",
                            self.researcher.websocket,
                        )
            ...
            if combined_context:
                ...
                if self.researcher.verbose:
                    ...
                    await stream_output(
                        "logs",
                        "context_combined",
                        f"[DATA] 合并研究上下文: {mcp_count} 条 MCP 来源{cache_status}, {'网络内容' if web_available else '无网络内容'}",
                        self.researcher.websocket,
                    )
            else:
            ...
            if self.researcher.verbose:
                await stream_output(
                    "logs",
                    "subquery_error",
                    f"[ERROR] 处理子查询 '{sub_query}' 时出错: {str(e)}",
                    self.researcher.websocket,
                )
            return ""
        ...
        try:
            ...
            if self.researcher.verbose:
                await stream_output(
                    ...
                    f"[THINK] 阶段 1: 为查询选择最优 MCP 工具: {query}",
                    self.researcher.websocket,
                )
            ...
            if results:
                ...
                if self.researcher.verbose:
                    await stream_output(
                        ...
                        f"[DONE] MCP 研究完成: 获取到 {result_count} 条智能结果",
                        self.researcher.websocket,
                    )
            ...
            if self.researcher.verbose:
                await stream_output(
                    "logs",
                    "mcp_research_error",
                    f"[WARN] MCP 研究出错: {str(e)} - 继续使用其他来源",
                    self.researcher.websocket,
                )
            return []
                    ...
                    if is_mcp_retriever:
                        if self.researcher.verbose:
                            await stream_output(
                                "logs",
                                "mcp_results",
                                f"[OK] 从 MCP 服务器获取了 {result_count} 条结果",
                                self.researcher.websocket,
                            )
