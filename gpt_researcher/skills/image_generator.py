    ...
    async def plan_and_generate_images(
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"[IMG][?] 识别出 {len(image_concepts)} 个可视化概念, 正在生成图片...",
                self.researcher.websocket,
            )
            ...
            try:
                if self.researcher.verbose:
                    await stream_output(
                        "logs",
                        "image_generating",
                        f"[IMG][?] 正在生成图片 {index + 1}/{len(image_concepts)}: {concept['title'][:50]}...",
                        self.researcher.websocket,
                    )
        ...
        if self.researcher.verbose:
            if generated_images:
                await stream_output(
                    "logs",
                    "images_ready",
                    f"[OK] {len(generated_images)} 张图片已就绪, 可以嵌入报告",
                    self.researcher.websocket,
                )
            else:
                await stream_output(
    ...
    async def generate_images_for_report(
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"[SEARCH] 发现 {len(suggestions)} 个适合添加图片的章节",
                self.researcher.websocket,
            )
        ...
        for i, suggestion in enumerate(suggestions):
            if self.researcher.verbose:
                await stream_output(
                    "logs",
                    "image_generating",
                    f"[IMG][?] 正在生成图片 {i+1}/{len(suggestions)}: {suggestion['section_header'][:50]}...",
                    self.researcher.websocket,
                )
                ...
                if images:
                    ...
                    if self.researcher.verbose:
                        await stream_output(
                            "logs",
                            "image_generated",
                            f"[OK] 已生成图片: {suggestion['section_header'][:50]}",
                            self.researcher.websocket,
                        )
            except Exception as e:
        ...
        if generated_images:
            ...
            if self.researcher.verbose:
                await stream_output(
                    ...
                    f"[DONE] 成功生成并嵌入 {len(generated_images)} 张图片",
                    self.researcher.websocket,
                )
    ...
    async def process_image_placeholders(
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"[ART] 发现 {len(placeholders)} 个图片占位符待处理",
                self.researcher.websocket,
            )
        ...
        for i, match in enumerate(placeholders):
            ...
            if self.researcher.verbose:
                await stream_output(
                    "logs",
                    "image_generating",
                    f"[IMG][?] 正在生成图片 {i+1}/{len(placeholders)}: {image_description[:60]}...",
                    self.researcher.websocket,
                )
        ...
        if generated_images and self.researcher.verbose:
            await stream_output(
                "logs",
                "image_generation_complete",
                f"[DONE] 成功生成 {len(generated_images)} 张内联图片",
                self.researcher.websocket,
            )
