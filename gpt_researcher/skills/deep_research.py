...
{{"questions": ["<question 1>", "<question 2>"]}}"""}
    ...
    async def deep_research(
        ...
        print(f"[SEARCH] Generating {breadth} search queries...", flush=True)
        serp_queries = await self.generate_search_queries(query, num_queries=breadth)
        print(f"[OK] Generated {len(serp_queries)} queries: {[q['query'] for q in serp_queries]}", flush=True)
        ...
        async def process_query(serp_query: Dict[str, str]) -> Optional[Dict[str, Any]]:
            async with semaphore:
                try:
                    ...
                    print(f"\n[ERROR] DEEP RESEARCH ERROR: {str(e)}\n{error_details}", flush=True)
                    return None
