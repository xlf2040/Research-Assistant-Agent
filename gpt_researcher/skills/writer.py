    ...
    async def write_report(self, existing_headers: list = [], relevant_written_contents: list = [], ext_context=None, custom_prompt="", available_images: list = None) -> str:
        ...
        if available_images and self.researcher.verbose:
            await stream_output(
                "logs",
                "images_available",
                f"[IMG][?] {len(available_images)} 张预生成图片可用于嵌入",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "writing_report",
                f"[WRITE][?] 正在撰写报告: '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "report_written",
                f"[NOTE] 已完成报告: '{self.researcher.query}'",
                self.researcher.websocket,
            )
    ...
    async def write_report_conclusion(self, report_content: str) -> str:
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "writing_conclusion",
                f"[WRITE][?] 正在撰写结论: '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "conclusion_written",
                f"[NOTE] 已完成结论: '{self.researcher.query}'",
                self.researcher.websocket,
            )
    ...
    async def write_introduction(self):
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"[WRITE][?] 正在撰写引言: '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"[NOTE] 已完成引言: '{self.researcher.query}'",
                self.researcher.websocket,
            )
    ...
    async def get_subtopics(self):
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"[TREE] 正在生成子主题: '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "subtopics_generated",
                f"[CHART] 已生成子主题: '{self.researcher.query}'",
                self.researcher.websocket,
            )
    ...
    async def get_draft_section_titles(self, current_subtopic: str):
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"[REPORT] 正在生成章节标题: '{self.researcher.query}'...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            await stream_output(
                ...
                f"[FILES][?] 已生成章节标题: '{self.researcher.query}'",
                self.researcher.websocket,
            )
