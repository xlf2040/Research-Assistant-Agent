...
class MCPRetriever:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Log initialization
        if self.mcp_configs:
            self.streamer.stream_log_sync(f"[MCP] Initializing MCP retriever for query: {self.query}")
            self.streamer.stream_log_sync(f"[MCP] Found {len(self.mcp_configs)} MCP server configurations")
        else:
            logger.error("No MCP server configurations found. The retriever will fail during search.")
            self.streamer.stream_log_sync("[ERROR] CRITICAL: No MCP server configurations found. Please check documentation.")
    ...
    def search(self, max_results: int = 10) -> List[Dict[str, str]]:
        ...
        if not self.mcp_configs:
            error_msg = "No MCP server configurations available. Please provide mcp_configs parameter to GPTResearcher."
            logger.error(error_msg)
            self.streamer.stream_log_sync("[ERROR] MCP retriever cannot proceed without server configurations.")
            return []  # Return empty instead of raising to allow research to continue
        ...
        try:
            ...
            logger.error(f"Error in MCP search: {e}")
            self.streamer.stream_log_sync(f"[ERROR] Error in MCP search: {str(e)}")
    ...
    async def _get_all_tools(self) -> List:
        ...
        try:
            ...
            if all_tools:
                await self.streamer.stream_log(f"[MCP] Loaded {len(all_tools)} total tools from MCP servers")
