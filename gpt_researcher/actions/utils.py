...
async def stream_output(
    ...
    if (not websocket or output_log) and type != "images":
        try:
            # Strip emoji/non-ASCII characters for console output on Windows
            safe_output = output.encode('ascii', errors='replace').decode('ascii') if output else output
            logger.info(f"{safe_output}")
        except (UnicodeEncodeError, UnicodeDecodeError):
            # Fallback: replace all non-ASCII characters
            logger.info("[log output contained non-printable characters]")
