...
def setup_research_logging():
    ...
    # Add stream handler for console output (use UTF-8 to avoid GBK encoding errors on Windows)
    import sys
    import io
    utf8_console_stream = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8', errors='replace') if hasattr(sys.stderr, 'buffer') else sys.stderr
    console_handler = logging.StreamHandler(stream=utf8_console_stream)
