...
class GPTResearcher:
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize a 科研agent助手 instance.
    ...
    async def _log_event(self, event_type: str, **kwargs):
        ...
        if self.log_handler:
            try:
                ...
                # Add direct logging as backup (ASCII-safe for Windows GBK console)
                import logging
                research_logger = logging.getLogger('research')
                try:
                    log_msg = f"{event_type}: {json.dumps(kwargs, default=str, ensure_ascii=True)}"
                    research_logger.info(log_msg)
                except (UnicodeEncodeError, UnicodeDecodeError):
                    research_logger.info(f"{event_type}: [log contained non-encodable characters]")
