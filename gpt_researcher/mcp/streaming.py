...
class MCPStreamer:
    ...
    async def stream_stage_start(self, stage: str, description: str):
        ...
        await self.stream_log(f"[STAGE] {stage}: {description}")

    async def stream_stage_complete(self, stage: str, result_count: int = None):
        """Stream the completion of a research stage."""
        if result_count is not None:
            await self.stream_log(f"[DONE] {stage} completed: {result_count} results")
        else:
            await self.stream_log(f"[DONE] {stage} completed")

    async def stream_tool_selection(self, selected_count: int, total_count: int):
        """Stream tool selection information."""
        await self.stream_log(f"[SELECT] Using LLM to select {selected_count} most relevant tools from {total_count} available")

    async def stream_tool_execution(self, tool_name: str, step: int, total: int):
        """Stream tool execution progress."""
        await self.stream_log(f"[EXEC] Executing tool {step}/{total}: {tool_name}")

    async def stream_research_results(self, result_count: int, total_chars: int = None):
        """Stream research results summary."""
        if total_chars:
            await self.stream_log(f"[DONE] MCP research completed: {result_count} results obtained ({total_chars:,} chars)")
        else:
            await self.stream_log(f"[DONE] MCP research completed: {result_count} results obtained")

    async def stream_error(self, error_msg: str):
        """Stream error messages."""
        await self.stream_log(f"[ERROR] {error_msg}")

    async def stream_warning(self, warning_msg: str):
        """Stream warning messages."""
        await self.stream_log(f"[WARN] {warning_msg}")

    async def stream_info(self, info_msg: str):
        """Stream informational messages."""
        await self.stream_log(f"[INFO] {info_msg}") 